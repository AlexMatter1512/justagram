...
async function build() {
  ...
  // 1. Discover Assets
  const cssGlobal = await getFiles(join(INJECTED_DIR, "css"), ".css");
  ...
  console.log(`Found ${cssGlobal.length} global CSS files`);
...
export const AssetManifest = {
  cssGlobal: ${JSON.stringify(cssGlobal)},
...
`;
  ...
  await mkdir(join(WWW_DIR, "injected", "css"), { recursive: true });
  ...
  // Injected CSS
  for (const css of cssGlobal) {
    await Bun.write(
      join(WWW_DIR, "injected", "css", css),
      Bun.file(join(INJECTED_DIR, "css", css))
    );
  }
